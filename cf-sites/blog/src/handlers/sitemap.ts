 ...
 * - /sitemap-posts.xml      → Legacy redirect → /sitemap.xml
...
export async function handleSitemap(env: Env, pathname: string): Promise<Response | null> {
  ...
  // Legacy: /sitemap-posts.xml used to be a nested sitemapindex — redirect to main index
  if (pathname === '/sitemap-posts.xml') {
    return Response.redirect(
      `${env.EFFECTIVE_ORIGIN || ''}/sitemap.xml`,
      301,
    );
  }
...
async function generateSitemapIndex(env: Env): Promise<Response> {
  const [config, postsResult, pages] = await Promise.all([
    getConfig(env.CONTENT_BUCKET, env.SITE_ID, env.CACHE),
    getPosts(env.DB, env.SITE_ID, 1, 1),
  ...
  const lastmod = postsResult.posts.length > 0 && postsResult.posts[0].publishedAt
    ? new Date(postsResult.posts[0].publishedAt).toISOString().split('T')[0]
    : undefined;
...
async function generatePostsSitemap(env: Env, page: number): Promise<Response> {
  ...
  const { posts } = await getPosts(
    env.DB, env.SITE_ID, page, MAX_URLS_PER_SITEMAP
  );
