...
- 如有 stub：读取规划笔记
- 如有相关已有 entries：Grep 搜索，读取 3-5 个最相关条目
...
5. 设置 coverImage：优先用 hero 图的 publicUrl，没有则用第一张图的 publicUrl
...
**关键**：所有图片类型都要上传（Stock 照片、SVG 插图、数据图表），CF Sites 不会自动下载本地路径引用的图片。
